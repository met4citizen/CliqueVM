...
self.onmessage = (msg) => {
  ...
  if ( d.action === 'setup' ) {
    if ( ws.length ) {
      ws.forEach( x => x.close() );
      ws.length = 0;
    }
    if ( d.server && Array.isArray(d.server) && d.server.length ) {
      ws = new Array(d.server.length);
      let connected = 0;
      for( let i=0; i<d.server.length; i++ ) {
        ws[i] = new WebSocket( d.server[i] );
        statuses[i] = 0;
        ws[i].onerror = (err) => {
          if ( reject ) reject();
          throw new Error('Web Socket connection to ' + d.server[i] + ' failed.');
        };
        ws[i].onmessage = (msg2) => {
          try {
            const m = JSON.parse(msg2.data);
            if ( m.task === task ) {
              m.data.forEach( (x,j) => {
                cliques[m.job+j] = x.map( x => x.map( y => V.get(y) ) );
              });
              job += m.data.length;
              if ( job === cliques.length ) {
                if ( resolve ) resolve();
                statuses[i] = 0;
              } else {
                if ( !postProgress( { action: "clique", job: m.task }, (1 + job / cliques.length ) / 2 ) ) {
                  if ( reject ) reject();
                  queue.length = 0;
                  task++;
                  statuses[i] = 0;
    ...
    throw new TypeError('Unknown action (' + d.action + ').');
  }
}
...
async function findCliques(d) {
  ...
  cliques = new Array(G.size);
  let arr = [...G.values()];
  let chunksize = Math.min( Math.ceil(arr.length / ws.length), 50 );
  let t = 0;
  job = 0;
  queue.length = 0;
  while( (t * chunksize) < arr.length ) {
    queue.push( JSON.stringify({
      action: 'task',
      task: task,
      job: t * chunksize,
      data: arr.slice( t * chunksize, (t+1) * chunksize ).map( x => x.map( y => y.id ) )
    }));
    t++;
  }
  ...
  for( let [coord,states] of G ) {
    ...
    // Report maximal cliques
    if ( model.probs ) {
      loc.probs = model.probs(coord, cliques[i]);
    }
