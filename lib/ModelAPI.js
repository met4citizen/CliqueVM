...
class ModelAPI {
  ...
  * @param {number} [observer=1] Number of observers
  * @return {number[]} Probabilities for each clique
  */
  probsPerm(cs, observer=1) {
    let tot = BigInt(0);
    let X = cs.map( x => {
      let y = this.factorial( x.length + observer );
      tot += y;
      return y;
    });
  ...
  * @return {number[]} Probabilities for each clique
  */
  probsComb(cs, observer=1) {
    let tot = cs.reduce( (a,b) => a+(b.length + observer), 0);
    return cs.map( x => (x.length + observer) / tot );
  }

  /**
  * Calculate probabilities without weights, but resolving conflicts.
  * @param {any[][]} cs Cliques i.e. array of array of states
  * @return {number[]} Probabilities for each clique
  */
  probsNone(cs) {
    ...
    for( let [s,cndxs] of m ) {
      const o = { s: s, cndxs: cndxs, m: m, d: 1 };
      jobs.push(o);
    }

    // Probabilities
    const ps = Array(cs.length).fill().map( x => [] );
    while(jobs.length) {
      const job = jobs.shift();
      if ( job.cndxs.length === 1 ) {
        ps[job.cndxs[0]].push( job.d * job.m.size );
      } else {
        ...
        for( let [s,cndxs] of m2 ) {
          const o = { s: s, cndxs: cndxs, m: m2, d: job.d * job.m.size };
          jobs.push(o);
        }
    ...
    return ps.map( x => x.reduce((a,b) => a+(1/b),0));
  }
