...
self.onmessage = (msg) => {
  ...
  if ( results.model.pass ) {
    ...
    if ( results.init.pass ) {
      ...
      testProbs(msg.data,results.probs);
...
// Test location
function testProbs(testset,status) {
  try {
    ModelAPI.prototype['location'] = new Function("coord","cs",testset['probs']);
    const ps = model.probs(coord,[is]);

    if ( ps && !Array.isArray(ps) ) {
      status.error = "ERROR: The returned value was not an array or null/undefined.";
    } else {
      status.pass = true;
    }
  }
